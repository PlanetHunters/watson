...
class Report:
    ...
    def create_header(self, canvas, doc):
        ...
        # Logo:
        canvas.drawImage(self.LOGO_IMAGE, x=0 * cm, y=26.8 * cm, height=2 * cm, width=2 * cm, preserveAspectRatio=True)

        # Title:
        object_id_text = 'PATOSO Data Validation Report: %s' % self.object_id
        canvas.setFont(psfontname="Helvetica", size=12)
        canvas.drawRightString(x=14 * cm, y=28 * cm, text=object_id_text)
        if doc.page == 1:
    ...
    def create_report(self):
        ...
        introduction = '<font name="HELVETICA" size="9">This document is generated by the PATOSO report generator (' \
                       '<a href="https://github.com/PlanetHunters/patoso">https://github.com/PlanetHunters/patoso</a>). ' \
                       'The target star this document focuses on is the %s. The star ' \
                       'parameters are shown in Table 1. The proposed candidate parameters are those exposed in ' \
                       'Table 2. The final section shows relevant single transits plots.</font>' % self.object_id
        ...
        story.append(Spacer(1, 15))
        story.append(Image(self.data_dir + "/transit_depths.png", width=16 * cm, height=10 * cm))
        table3_descripcion = '<font name="HELVETICA" size="9"><strong>Table 2: </strong>\
        ...
        introduction = '<font name="HELVETICA" size="9">The next pages will contain each of the single-transits ' \
                       'vetting sheet with the next information: </font>'
        story.append(Paragraph(introduction, styles["ParagraphAlignJustify"]))
        story.append(ListFlowable(
            [Paragraph(s, styles["ParagraphAlignJustify"]) for s in [
                "TOP-LEFT: Plot with single transit photometry found in the analyzed curve (with momentum dumps, if any).",
                "TOP-CENTER: Plot with X-axis data drift (X-axis motion vs X-axis centroid offset) around the transit times.",
                "TOP-RIGHT: Plot with Y-axis data drift (Y-axis motion vs Y-axis centroid offset) around the transit times.",
                "CENTER-LEFT: Plot with SAP for used aperture vs SAP for smaller aperture around the transit times.",
                "CENTER-CENTER: Plot with smaller aperture over used aperture on the target.",
                "CENTER-RIGHT: Plot with single transit photometry found in the analyzed curve around the transit times.",
                "BOTTOM: Plot TPF flux measurements for each pixel around the transit times.",
            ]],
            bulletFormat="%s.",  # ←
        ))
